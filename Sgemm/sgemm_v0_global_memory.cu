...
__global__ void sgemm_globalMemory(float* d_A,float* d_B,float* d_C,const int M,const int N,const int K){
    int m_index = blockDim.x*blockIdx.x + threadIdx.x;
    int n_index = blockDim.y*blockIdx.y + threadIdx.y;
    if(m_index < M && n_index < N){
        float sum = 0.0f;
        for(int k = 0;k<K;k++){
            sum += d_A[m_index*K+k]*d_B[k*N+n_index];
        }
        d_C[m_index*N+n_index] = sum;
    }
}
