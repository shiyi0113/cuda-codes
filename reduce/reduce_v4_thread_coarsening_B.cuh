...
// reduce threads by half block
__global__ void reduce4B(float *d_input,float *d_output){
    __shared__ float s_input[THREAD_PER_BLOCK / 2];
...
void reduce4B_thread_cosrsening(){
    ...
    dim3 Block(THREAD_PER_BLOCK/2,1);
    reduce4B<<<Grid,Block>>>(d_input,d_output);
